...
**Where rows go.** The log's table is deliberately its LAST section (restructured 2026-08-05): append new rows at the end of the file and they land inside the table by construction. Do not insert rows above the narrative sections.
...
**A status line that names live external state is re-read, never carried forward.** When a handoff's status asserts something only the outside world can confirm - which version is published, which banner is on the profile, what a remote serves, what is deployed - do not copy the prior status into the refreshed one. Open the canonical file that records that state and read it. Carrying it forward launders a stale claim into a fresh-looking one, because the refreshed date says the whole line was checked when only the file's mtime was. Evidence (2026-09-11): `personal-brand-strapline-and-banner-ab.md` was written 2026-08-05 naming banner v3e as live; the v4, v5 and v6 tracks all shipped afterwards; the 2026-09-08 backlog sweep refreshed its status line without opening `Brand/brand-tokens.md`, which had recorded v6a 'comet' as live since 2026-08-18. A session five weeks later trusted the handoff, and the conflict cost a round trip plus a failed attempt to read the live profile (LinkedIn answers HTTP 999 to unauthenticated fetches). The cheap form of this check is one grep of the canonical file per external claim, and it is cheaper than the handoff being wrong.
...
1. **Finished this session.** If this session closed a workstream that has a handoff, delete that file (`git rm`) and report it. A handoff for completed work is a stale-instruction hazard. **Exception — keep it when it carries decisions recorded nowhere else.** Before deleting, check whether the file holds a `Decisions already locked` section whose entries have no `recorded in` pointer to a surface outside the handoff. If it does, the delete destroys the reasoning behind settled calls, which is the top waste category in Ane's own session-efficiency protocol: a decision without its reason gets re-argued the moment it looks inconvenient. Keep the file and **neutralise it instead**, which costs three edits: open the status line with `CLOSED <date>`, add a line under it saying the file is a record and not an instruction, and strike every entry under `Next step`. **That marker goes in the `Workstream: ... Status:` line and nowhere else.** `handoff_status.py` reads closure out of that one line and never looks at headings, so a file announcing `## DONE` or `## CLOSED` in a heading still reports as open and keeps drawing the stale prompt at every scan. Evidence (2026-09-08, handoff backlog sweep): `melai-pilot-implementation-plan-update.md` opened with `## DONE - 7 August 2026. Kept for the decision record; nothing here is outstanding.` and was still listed OPEN at 32 days, because the first `Workstream: ... Status:` line in the file sat lower down inside a preserved original-handoff section and still read `fully specified, not started`. Where a file preserves its original handoff verbatim, add a FRESH `Workstream: ... Status: CLOSED <date>` line ABOVE it rather than editing the historical one: the scan takes the first match, so the history stays intact and the scan reads the truth. That removes the stale-instruction hazard the delete rule exists to prevent while keeping what only this file knows. Report it as `kept — carries N locked decisions`, not as a deletion. Evidence (2026-08-15, two consecutive sessions): `system-audit-follow-ups.md` closed carrying 16 locked decisions and `wiki-routing-and-lint-followups.md` carrying 5, and both sessions overrode the delete rule on the same reasoning. An override taken twice and written down nowhere is one the third session forgets.
2. **Flagged stale (7 days or older, no activity).** Do NOT delete it. Show Ane the line and ask once: `close it? (y/n)`. On yes, `git rm` it. On no or no answer, leave it and move on. Handoffs whose status opens `CLOSED` are not stale and never carry this prompt — `handoff_status.py` files them under `CLOSED — kept as a record, no action`. Before that classifier existed both closed files sat in the open list offering to close what was already closed, which is the one prompt guaranteed to train a reader to answer without looking.

**Date the closing write, or the scan will not see it.** `handoff_status.py` takes a handoff's age from the FIRST `written by /wrap-up on <date>` comment, never from mtime, because OneDrive and git both rewrite mtime without the file changing. So a close-out that edits only the body leaves the file reporting its original age: on 2026-08-15 a handoff closed that afternoon still read `12 days ⚠️ no activity`. Update that comment to today whenever you write to a handoff, and keep the original authoring date inside the comment body.
