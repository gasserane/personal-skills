...
5. **Three traps seen on 2026-09-24 (HERA AAR round).** (a) Pass Windows paths (`C:/Users/...`) to the driver; a Git Bash path (`/c/Users/...`) raises `FileNotFoundError` inside `zipfile`. (b) `read_comments` joins a comment's paragraphs with no separator, so a comment written with `
