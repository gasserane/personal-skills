...
5. **Traps seen since 2026-09-24 (HERA AAR round onward).** (a) Pass Windows paths (`C:/Users/...`) to the driver; a Git Bash path (`/c/Users/...`) raises `FileNotFoundError` inside `zipfile`. (b) `read_comments` joins a comment's paragraphs with no separator, so a comment written with `
` reads back run-together; compare whitespace-stripped text, or a correct write reports as a mismatch. (c) The driver can only ADD comments. Rewriting Ane's existing comment text in place is not supported yet (backlog: ENHANCE office-review-pass). Until it is, keep each comment's LAST `w:p`, because `commentsExtended.xml` and `commentsIds.xml` key on that paragraph's `w14:paraId`, and rebuild the text around it. Change `w:author` in both `comments.xml` and `people.xml`, verify ids, authors, dates, anchors and body word count on a scratch copy, then back up and swap. Reference script: `agent-improvements/handoffs/hera-hub-round2-scripts/hera_apply_aar.py` in the work folder. (d) *Added 2026-09-25 (RHNK Kenya case study):* anchors match character for character, and Word text usually carries curly apostrophes and quotes (`’`, `“`). A straight `'` typed into the match string finds nothing. Pick five-word spans without apostrophes or quotes, or copy the span from the extracted text rather than retyping it. The driver pre-check names the failing anchor, but choosing clean spans up front avoids the re-run. (d) **For a wording change, edit inside the existing `w:t` runs instead of rebuilding paragraphs**: find the one `w:t` that contains the old string exactly once, replace it there, and fail closed on zero or several hits. Paragraph count, paraIds and run formatting then stay untouched, and every comment you did not change can be asserted byte-identical in `comments.xml`. Reference: `hera-hub-round2-scripts/hera_apply_align.py` (2026-09-24, run twice). (e) **A Word re-save can split a comment's text across runs** without changing a character of it: after Ane re-saved the AAR, one sentence broke at `Table| 4)` and the single-run match found nothing. Fail closed, dump the run structure, re-target the edit at the run that holds it; never loosen the match or merge runs.
...
4. **Read the returned counts and report them.** The function reads them back out of Word rather than assuming. Any pair returning `0` is a wrong search string and must be raised with Ane, not quietly dropped. Pass `--headers` when the text also sits in the header or footer, which is where contact blocks and document titles hide. **A count of 1 for a string you know appears twice is a real miss, not a rounding.** On 2026-09-24 a header repeated in two tables changed only in the first; re-rendering the file showed the second unchanged, and a second run caught it. Re-render and diff against the pre-edit rendering; never trust the count alone. When the same cell text repeats and must change differently per row (`Yes` → `Yes (Table 1, EQ3)`), find-replace cannot target by position: edit the cells by table, row and column with python-docx instead, and see the next paragraph for how to save it.
...
**Saving a python-docx edit into Ane's file.** `Document.save()` re-serialises EVERY part: header, footer, styles, settings, `core.xml`. A byte check against the original fails on all of them even when the body edit is all you made. Save to a temporary file, take only its `word/document.xml`, and write it into a copy of the ORIGINAL package with every other part byte-identical. Then assert that nothing but `word/document.xml` differs, back up, and swap. Reference: `hera-hub-round2-scripts/hera_annex_final_ir.py`.
