...
description: 'Take a Word deliverable through a review and revision cycle without losing Ane''s formatting. Four modes: read (extract the body with her comments anchored in place, then an independent expert analysis that engages with those comments), track (apply edits as real Word tracked changes a counterparty can accept or reject), revise (scoped formatting-preserving fixes to a hand-edited .docx via Word COM), annex (branded glossary plus an annex of referenced documents and laws as session-verified hyperlinks). Use whenever Ane says "review this document", "what do you make of my comments", "read the comments on this docx", "reply to the reviewer", "apply these as tracked changes", "mark this up so they can accept or reject", "fix these points in the Word file", "make these edits without breaking the formatting", "add a glossary and a source annex", or hands over a .docx that someone has commented on or that needs edits applied. Reach for it whenever the work happens INSIDE a .docx rather than in chat. Distinct from ane-voice (English prose voice only, never touches the file), localise (target-language work), check-deliverable (a read-only QA gate that returns a verdict, not an edited file), and tor-procurement (builds ToRs from a generator).'
model: opus
---
