...
description: 'Build the Excel toolkit that runs a selection, between a published ToR and the award: procurement mode (master workbook plus one scorer workbook per panel member) and appraisal mode (generic multi-criteria decision analysis), off one weighted scoring engine. Use whenever Ane says "selection toolkit", "scoring grid for this ToR", "set up the panel scoring", "evaluation workbook", "run the selection for X", "build the scoring sheets", "panel scoring files", "weighted scoring matrix", "multi-criteria appraisal", "score these options against criteria", or hands over a published ToR and asks what the panel scores in. Distinct from tor-procurement (writes and grades the ToR, and stops at publication), procurement-offer-review (analyses one incoming offer against a ToR), donor-proposal-scoring (scores proposals IPPF sends out rather than receives), and accreditation-desk-review (Member Associations against IPPF standards).'
model: opus
---
