...
description: 'Build the Excel toolkit that runs a selection, between a published ToR and the award. Two modes off one weighted scoring engine. Procurement mode takes the published award criteria of an open-market procurement and emits one master workbook for the administrators plus one standalone scorer workbook per panel member, with a compliance grid, a question log and publishable Q&A extract, panel-mean technical scoring, lowest-compliant-offer financial scoring, a conflict-of-interest register, a decision log and applicant feedback. Appraisal mode runs the same engine as a generic multi-criteria decision analysis for an options, tool, vendor or value-for-money comparison. Use whenever Ane says "selection toolkit", "scoring grid for this ToR", "set up the panel scoring", "evaluation workbook", "run the selection for X", "build the scoring sheets", "panel scoring files", "weighted scoring matrix", "multi-criteria appraisal", "score these options against criteria", or hands over a published ToR and asks what the panel scores in. Every criterion, weight, threshold and date is read from the source or reported as a gap; nothing is invented. Distinct from tor-procurement (writes and grades the ToR, and stops at publication), procurement-offer-review (analyses one incoming offer against a ToR), donor-proposal-scoring (scores proposals IPPF sends out rather than receives), and accreditation-desk-review (Member Associations against IPPF standards).'
model: opus
---
