...
description: 'Prepare the first working session with a contracted supplier, against a fixed day budget. Reads the agreed offer or contract for the days, rates and tranches it bought, then builds the session pack, a branded workbook whose day-budget sheet totals against the contracted ceiling and shows the remainder, plus an internal prep brief, an agenda for the supplier and a post-session note template. The value is the arithmetic, not the documents, because it makes over-scope visible in month one rather than month five. Use whenever Ane says "prep my kick-off with [supplier]", "first work session with [name]", "batch 1 planning", "delivery session prep", "prepare for the session that starts [supplier] work", "how do we split the contracted days", "we are over the day budget", or hands over a signed offer and asks what to run the first session on. Every day, rate and tranche is read from the contract or reported as a gap; nothing is invented. Distinct from tor-procurement (writes and grades the ToR, and stops at publication), selection-toolkit (runs the panel scoring between publication and award), procurement-offer-review (analyses one incoming offer), implementation-pack (tracks a donor grant IPPF receives rather than a contract IPPF issues), and meeting-notes mode 2 (manager one-to-one prep, untouched by this).'
model: opus
---
