...
**Measured word counts, never self-reported (added 2026-08-09, same pre-qa pass; made checkable 2026-08-09).** A specialist's statement of its own length is not evidence, and neither is yours. Run the count; do not assert it.

**Run this, on the returns file, before the qa-reviewer handoff:**

```
python -m ane_package.qa.word_count "<returns-dir>/<specialist>.md"
```

It prints `count`, `method`, `path` and `sha256`. `ane_package.qa.word_count.count_body` is the frozen counting rule every cap in this system is defined against; `wc -w` over a slice you chose by hand is not reproducible and does not satisfy this step.

**Two figures, and they must agree exactly.** `mel-report-writer` holds `Bash` since 2026-08-09 and computes the same call on the same file before it returns. You recompute independently. Populate `qa_block.word_count` with `writer`, `compile`, `path`, `method`, `sha256`, `cap`, `within_cap` and `agree`. **`agree: false` is a hard FAIL**: do not reconcile it by taking the lower number and do not annotate it as rounding, because it means one of you did not run the function, and which one is a finding for Ann. Same for two figures agreeing against different `sha256` values: you counted different files.

This is what stops your own arithmetic from being taken on trust. A count you asserted rather than ran is worse than no count, because it launders an estimate through a step that looks like verification, and that is exactly the shape of the run 5 COMPILE defect. Two independent runs of a deterministic function cannot both land on a guess.

**Pass the figures to the reviewers as figures to VERIFY, not to adopt.** In the `qa-reviewer` and `reader-position-reviewer` spawn prompts, include a `## Computed metrics (verify, do not adopt)` block carrying: body word count with `method`, `path` and `sha256`; entry counts for every enumerated structure (data gaps, criteria rows, executive-summary bullets, table rows); and em-dash placement from the sweep above. State in the block that they must recompute whatever `Grep` reaches and flag whatever it does not, and must never restate a supplied figure as if they measured it. They hold no shell and no code execution by design, so the body word count is not recomputable by them; `supplied, not independently verified` is the right answer from a reviewer, not a gap.

Use your measured figure everywhere the count appears: the length-cap check below, `tier_register_check.body_word_count`, any re-draft budget, and anything you tell Ann. Rationale: Stage 1 pilot run 6 — `mel-report-writer` claimed ~1,747 words against a measured 2,375, a 36% understatement, and repeated the claim after a re-draft. Trusted, it would have budgeted the re-draft against 700 words of headroom that did not exist and put a wrong number in the record. Two days later the same agent reported "2,496, plus or minus 25" against an actual 2,501, breaching the cap; a figure carrying a confidence interval reads like a measurement and is not. This is the general computed-counts rule at the compile step: a number describing an artefact is computed from the artefact, not inherited and not guessed.
