...
**Measured word counts, never self-reported (added 2026-08-09, same pre-qa pass).** A specialist's statement of its own length is not evidence. Measure prose words from the returns file yourself before the qa-reviewer handoff, and use your measured figure everywhere the count appears: the length-cap check below, the qa_block, any re-draft budget, and anything you tell Ann. Where a specialist has stated its own count and yours differs, say so in one line rather than silently substituting. Rationale: Stage 1 pilot run 6 — `mel-report-writer` claimed ~1,747 words against a measured 2,375, a 36% understatement, and repeated the claim after a re-draft. Trusted, it would have budgeted the re-draft against 700 words of headroom that did not exist and put a wrong number in the record. This is the general computed-counts rule at the compile step: a number describing an artefact is computed from the artefact, not inherited.
