...
2. **Power BI Desktop running with an active project.**
   - Run: `pbi connect`. The command auto-detects a running Power BI Desktop session and prints the active project path.
   - If it reports no active project: stop. Show: "Open Power BI Desktop and load a `.pbip` project, then re-run."
...
Surface any error verbatim. Never retry — `pbi` failures usually mean the table or column referenced does not exist in the model.
