...
2. **Transcript** — from `ytInitialPlayerResponse.captions.playerCaptionsTracklistRenderer.captionTracks[]`, take the `baseUrl` of the wanted track and fetch it (`&fmt=json3` for JSON). **Known failure:** YouTube increasingly requires a POT token on caption URLs; the fetch returns empty (HTTP 200, 0 bytes) or 403.
3. **yt-dlp fallback (proven 2026-08-05, wayfinder scan)** — when the caption URL comes back empty, a local yt-dlp recovers the full auto-caption track where the raw fetch cannot: `"<WORK_FOLDER_ROOT>/local_analyst/.venv/Scripts/yt-dlp.exe" --skip-download --write-auto-subs --sub-langs en --sub-format json3 -o "<scratchpad>/<slug>" "<watch URL>"`, then parse `events[].segs[].utf8` from the json3. Two gotchas: give `-o` an explicit absolute path into the scratchpad, because the sandbox temp cwd is discarded between calls and the file otherwise lands somewhere unreachable (or worse, at the repo root); and the "impersonation" warning is harmless for subtitle-only runs. Only if yt-dlp is also unavailable: work from the description + chapters and say so in the roadmap's Source section — a scan of chapters is honest; a reconstructed transcript is not.
