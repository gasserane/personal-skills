...
2. **CURATE saturation check (added 2026-04-29; thresholds revised 2026-04-29 evening for session-retrospective baseline).** Check size of all four overlay files (`agent-improvements/{ann,vi,li,researcher}-overlay.md`). If any exceeds 16KB hard threshold OR 5+ COMPLEX runs since the most recent CURATE entry on that overlay: surface a banner in the first session output for that invocation: `🔔 [overlay-name] reached saturation threshold (X KB / N runs since last CURATE) — recommend /li curate to consolidate.` Soft warning at 13KB. Do NOT auto-execute CURATE — the banner is a recommendation only; CURATE is a destructive consolidation that archives entries, so Ane should approve before it runs. Add the overlay name to the `🔔 Flag for Ann:` block when triggered. Rationale: system audit 2026-04-28 flagged "No CURATE auto-trigger. Saturation is manual." as a Medium-severity quality leakage gap; ann-overlay.md grew past the original 12KB threshold on 2026-04-29 after the system-performance-review additions. Original 12KB threshold was sized for short STANDING PREFERENCE entries (~500 chars); session-retrospective entries average ~1500 chars, so 10 entries × ~1500 chars ≈ 15KB stable state — threshold raised to 16KB to give 1KB headroom over count-10 stable state (count-10 cap added to Step 9 same date).
