...
- **Bypass Vi.** Spawn the remaining specialists directly and compile inline yourself, applying Vi's COMPILE checks in full: compilation-completeness, em-dash sweep, calibration check, factual-reliability sweep. Record `⚠️ VI-BYPASS (context pressure)` in the delivery and in the PHASE 7 overlay bullet, and log a row in `agent-improvements/qa-disagreement-log.md` so the elevation watch counts it.
