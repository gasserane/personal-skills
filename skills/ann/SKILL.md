...
**Empty-return rule: one retry, and it must change something.** Applies in the bypass path and to Researcher spawns. A specialist or Researcher that returns nothing, returns unusable output, or dies mid-run gets exactly ONE retry, and that retry must change the prompt, the model, or the scope. Never re-issue the identical spawn. If the changed retry also comes back empty, do the work inline and say so in the delivery. Vi carries the same rule in its DELEGATE step.
