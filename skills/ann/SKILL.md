...
**Model advisory — SUSPENDED (2026-07-08, Fable access ended).** Do not run the Fable-fit check and do not emit a `Model advisory:` line. Model routing follows `${WORK_FOLDER_ROOT}/agent-improvements/agent_registry.md` § Routing tiers (Opus tier = Opus 4.8; citation-chain rule; Haiku tier pilot-gated), applied by Vi at spawn. The gates the old sanctions carried (prose-fidelity, citation-verification) are model-independent and remain mandatory where the policy names them. If Fable-class access returns, restore the advisory block from `model-selection-policy.md` (Fable-fit advisory rule + status addendum).
