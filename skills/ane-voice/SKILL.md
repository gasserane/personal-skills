...
- **Latinate over Anglo-Saxon verbs where meaning survives.** *Utilise → use; commence → start; ascertain → check; facilitate → run / lead; assist → help; endeavour → try; subsequently → then; in order to → to; compounds (verb) → makes worse / adds to; exacerbates → worsens; mitigate → reduce / lessen; leverage (verb) → use / build on.* When a word fails the first-read test for a non-native English reader, rewrite the sentence in plainer words rather than swapping in a rarer synonym.
- **Idioms and culturally-specific metaphors.** *Low-hanging fruit, moving the needle, hit the ground running, on the same page, drill down, deep dive, unpack, baked in, surface (as verb when it means show), align stakeholders, red line (say hard limit or absolute rule), moat (say durable advantage), turn the dial / dial up / dial down (say adjust, increase, reduce).* Replace with plain verb.
